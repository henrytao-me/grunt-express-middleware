...
exports.runServer = function runServer(grunt, options) {
    ...
    // ekit disable original middleware
    ...
    if(options.logger){
    	if(options.logger === true){
    		options.logger = function(req, res, next) {
	            var info = {
	                start: new Date(),
	                end: null,
	                method: req.method,
	            ...
	            res.end = function(jObj) {
	                info.end = new Date();
	                info.statusCode = res.statusCode;
	                console.log('[' + info.start + '] ' + info.method + ' ' + info.url + ' - ' + info.statusCode + ' - ' + (info.end - info.start) + 'ms');

	                if (info.statusCode == 200) {
	                    try {
	                        if (res._headers['content-type'] === 'application/json; charset=utf-8') {
	                            var tmp = JSON.parse(jObj);
	                            if (tmp.status !== 'ok') {
    	...
    	options.logger.middlewarePriority = -10;
    	server.use(options.logger);
    };
    ...
    (function startServer(port) {
        ...
        var args = [port,
            function() {
                server.emit('startListening', this, server);
